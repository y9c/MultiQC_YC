...
class MultiqcModule(BaseMultiqcModule):
    def __init__(self):
        ...
        self.num_orgs = 0
        for f in self.find_log_files("fastq_screen_fork", filehandles=True):
        ...
        self.add_section(
            name="Mapped Reads (Heatmap)",
            anchor="fastq_screen_mapped_reads",
            plot=self.fqscreen_heatmap_plot(),
        )
        if (
            len(num_orgs) == 1
            and len(self.fq_screen_data) * self.num_orgs <= 600
            and not config.plots_force_flat
            and not getattr(config, "fastqscreen_simpleplot", False)
        ):
            self.add_section(
                name="Mapped Reads (Barplot)",
                anchor="fastq_screen_mapped_reads",
    ...
    def parse_fqscreen(self, f):
        ...
        for l in f["f"]:
            ...
            if l.startswith("%Hit_no_genomes:") or l.startswith(
                ...
                parsed_data["No hits"] = {
                    "percentages": {"one_hit_one_library": nohits_pct}
                }
            else:
                s = l.strip().split("\t")
        ...
        if (
            parsed_data.get("total_reads") is not None
            and nohits_pct is not None
        ):
            parsed_data["No hits"]["counts"] = {
                "one_hit_one_library": int(
                    (nohits_pct / 100.0) * float(parsed_data["total_reads"])
                )
            }
        else:
            log.warning(
    ...
    def parse_csv(self):
        ...
        for s in sorted(self.fq_screen_data.keys()):
            ...
            for org in self.fq_screen_data[s]:
                ...
                try:
                    k = "{} counts".format(org)
                    totals[s][k] = self.fq_screen_data[s][org]["counts"][
                        "one_hit_one_library"
                    ]
                    totals[s][k] += self.fq_screen_data[s][org]["counts"].get(
                        "multiple_hits_one_library", 0
                    )
                    totals[s][k] += self.fq_screen_data[s][org]["counts"].get(
                ...
                try:
                    ...
                    totals[s][k] += self.fq_screen_data[s][org][
                        "percentages"
                    ].get("multiple_hits_one_library", 0)
                    totals[s][k] += self.fq_screen_data[s][org][
                        "percentages"
                    ].get("one_hit_multiple_libraries", 0)
                    totals[s][k] += self.fq_screen_data[s][org][
                        "percentages"
                    ].get("multiple_hits_multiple_libraries", 0)
                except KeyError:
                    pass
        return totals

    def fqscreen_heatmap_plot(self):
        data = []
        xcat = []
        ycat = []
        for k, v in sorted(self.fq_screen_data.items()):
            if k not in ycat:
                ycat.append(k)
            l = []
            for k2, v2 in v.items():
                if k2 != "total_reads" and k2 != "No hits":
                    if k2 not in xcat:
                        xcat.append(k2)
                    l.append(
                        v2["percentages"]["one_hit_one_library"]
                        + v2["percentages"]["one_hit_multiple_libraries"],
                    )
            data.append(l)
        ...
        if len(data) > 0:
            plot = heatmap.plot(
                ...
                ycats=ycat,
                pconfig={
                    "id": "fastq-screen-heatmap-plot",
                    "title": "Sample source inference",
                    "min": 0,
                    "max": 100,
                    "square": True,
                },
    ...
    def fqscreen_plot(self):
        ...
        FastQ Screen program.

        Not useful if lots of samples as gets too wide.
        """
        ...
        """.format(
            ...
            dict=json.dumps(
                {"plot_id": plot_id, "data": data, "categories": categories}
            ),
        )
        ...
        for s_name in sorted(self.fq_screen_data):
            ...
            for org in self.fq_screen_data[s_name]:
                if org == "total_reads":
                    continue
                try:
                    data[s_name][org] = self.fq_screen_data[s_name][org][
                        "counts"
                    ]["one_hit_one_library"]
                except KeyError:
                    log.error(
    ...
    def fqscreen_bisulfite_plot(self):
        ...
        cats = OrderedDict()
        cats["original_top_strand"] = {
            "name": "Original top strand",
            "color": "#80cdc1",
        }
        ...
        cats["original_bottom_strand"] = {
            "name": "Original bottom strand",
            "color": "#dfc27d",
        }
        ...
        for s_name, d in self.fq_screen_data.items():
            for org, dd in d.items():
                ...
                try:
                    ...
                    total_counts = sum(
                        [c for c in dd["bisulfite_counts"].values()]
                    )
